...
WHERE
	XX.rnum = 1;
--OBTIENE EL ÛŒTIMO EVENTO NO_RECICLABLE  EN TODA LA HISTORIA HASTA LA FECHA DE PROCESO
DROP TABLE ${ESQUEMA_TABLA}.OTC_T_ALTA_BAJA_REPROCESO_HIST_UNIC;

CREATE TABLE ${ESQUEMA_TABLA}.OTC_T_ALTA_BAJA_REPROCESO_HIST_UNIC AS
SELECT
	XX.TIPO
	, XX.SUB_MOVIMIENTO
	, XX.TELEFONO
...
FROM
	...
	FROM
		${ESQUEMA_TABLA}.OTC_T_ALTA_BAJA_REPROCESO_HIST AS AA
	WHERE
		FECHA < '${fecha_movimientos}'
		--AND TIPO = 'ALTAS_BAJAS_REPROCESO'
    ) XX
WHERE
	XX.rnum = 1;
...
SELECT
	...
	, E.COD_PLAN_ANTERIOR AS CODIGO_PLAN_ANTERIOR
	, E.DES_PLAN_ANTERIOR AS DESCRIPCION_PLAN_ANTERIOR
	...
	, A.PROVINCIA_IVR
	, A.PROVINCIA_MS
	, B.LINEA_NEGOCIO_BAJA AS LINEA_DE_NEGOCIO_ANTERIOR
...
LEFT JOIN db_cs_altas.otc_t_altas_bi AS A_MES_ANT 
    ON
...
LEFT JOIN db_cs_altas.otc_t_transfer_out_bi AS TO_MES_ANT 
    ON
...
FROM
	...
	UNION ALL
		SELECT
			...
			, CAST(NULL AS STRING) AS PROVINCIA_USUARIO
		FROM
			${ESQUEMA_TABLA}.OTC_T_ALTA_BAJA_REPROCESO_HIST_UNIC
		WHERE
			FECHA BETWEEN '${f_inicio}' AND '${fecha_proceso}'
			-----------------------*************************-----------------
	UNION ALL
...
FROM
	db_desarrollo2021.pivot_movi_parque AS B
...
----- PROCESO
--sh -x /home/nae108834/Cliente360_RF/bin/D_OTC_T_360_MOVIMIENTOS_PARQUE.sh 20220913
----- REPROCESO
